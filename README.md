...
| CLI | ~90% | Stable flags (`-o`, `--human`, `--defects`, `--diff`, `--mode`, `--profile`, `--dry-run`, `--version`), the read-only `validate` subcommand, JSON reports, batch summary, exit codes. Plus an experimental prototype flag `--experimental-join-components` (moves small components onto the nearest larger one instead of deleting them; changes geometry, CLI-only, evaluation only). The `--human` report is English-only (localization is a GUI concern). |

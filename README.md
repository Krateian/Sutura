...
| Indirect predicates / Phase B arrangement-lite | ~30% — experimental | A Rust-based prototype (`--experimental-indirect-autorefine`) that splits self-intersecting geometry with exact indirect predicates instead of the float64 EPICK snap-rounding used by `--experimental-autorefine`. B1–B5 are implemented (predicate core, exact triangle–triangle classifier, 2D CDT with implicit points, `arrangement_lite` PyO3 binding, `sutura/repair.py` wiring). The "constrained edge blocks segment insertion" panic on dense multi-segment host triangles was fixed by switching to a planar arrangement/PSLG approach. Known limitations: disabled by default; the subdivision phase is exact-rational only, so dense self-intersection scans (e.g. thingi10k_1038441) do not finish in practical time; a filtered f64/interval/exact evaluation layer is required before it can replace the existing autorefine path. |

...
### Triage Engine intensity presets
...
four read-only presets, selected with `--intensity {quick,balanced,thorough,extreme}`
(or `SUTURA_INTENSITY`, or the `intensity` key of
...
their previous behaviour in every preset. The chosen preset is reported as
...
every preset. A future named-profile feature is designed to override any
preset field on top of a base preset; it is not implemented yet.
...
| Triage intensity (`--intensity`) | ~75% (new) | Four read-only presets (`quick`/`balanced`/`thorough`/`extreme`) for the post-Stage-1 effort (deep-repair tier, fTetWild state/cap/budget, decimation ladder, Hausdorff samples), a single source of truth in `sutura/triage.py`. Balanced equals the historical module constants and is regression-tested byte-identical; the CLI/GUI precedence is CLI > `SUTURA_INTENSITY` > config > balanced. Quick/Thorough/Extreme double as a way to bound or extend the two hard failure modes (fTetWild input size, dense-boundary decimation). Caveat: the preset values are engineering choices, not corpus-calibrated; a named user-profile override layer is designed but not implemented. |
...
- **Unreleased** — Triage Engine **intensity presets**
  (`--intensity quick|balanced|thorough|extreme`, GUI combo on the Options
  *Repair* tab with a *Reset to recommended* button): a single layer for the
  ...
  historical defaults; Stage 1 is unaffected.

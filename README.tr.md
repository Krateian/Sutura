...
### Triage Engine yoğunluk presetleri
...
okunur presette toplanmıştır: `--intensity {quick,balanced,thorough,extreme}`
(veya `SUTURA_INTENSITY`, veya `~/.config/sutura/config.json` içindeki
`intensity` anahtarı; bayrak, sonra ortam, sonra config kazanır). Aşama 1
bilinçli olarak **etkilenmez**: onarım modu (`--mode`/sınıflandırıcı) ve
isteğe bağlı profil (`--profile`) her preset'te önceki davranışını korur.
Seçilen preset raporda `triage_intensity` olarak ve `--human` çıktısında bir
`Intensity:` satırı olarak görünür.
...
değildir: her preset'te açık kalır. Gelecekteki adlandırılmış-profil özelliği,
bir taban preset üzerine her preset alanını geçersiz kılacak şekilde
tasarlanmıştır; henüz uygulanmamıştır.
...
| Triage yoğunluğu (`--intensity`) | ~%75 (yeni) | Aşama 1 sonrası çaba için dört salt-okunur preset (`quick`/`balanced`/`thorough`/`extreme`) — derin-onarım katmanı, fTetWild durumu/sınırı/bütçesi, sadeleştirme merdiveni, Hausdorff örnekleri — tek doğruluk kaynağı `sutura/triage.py`. Balanced tarihsel modül sabitlerine eşittir ve bayt-birebir regression testiyle doğrulanır; CLI/GUI önceliği CLI > `SUTURA_INTENSITY` > config > balanced. Quick/Thorough/Extreme iki zor başarısızlık modunu (fTetWild girdi boyutu, yoğun-sınır sadeleştirmesi) sınırlamak veya genişletmek için de kullanılır. Uyarı: preset değerleri mühendislik tercihleridir, korpus-kalibreli değildir; adlandırılmış kullanıcı-profili geçersiz kılma katmanı tasarlanmış ama uygulanmamıştır. |
...
- **Unreleased** — Triage Engine **yoğunluk presetleri**
  (`--intensity quick|balanced|thorough|extreme`, Seçenekler *Repair*
  sekmesinde GUI açılır listesi ve bir *Önerilene sıfırla* düğmesi): Aşama 1
  sonrası çaba için tek katman (derin-onarım katmanı, fTetWild
  durumu/sınırı/bütçesi, sadeleştirme merdiveni, Hausdorff örnekleri).
  Balanced tarihsel varsayılanlarla bayt-birebirdir; Aşama 1 etkilenmez.

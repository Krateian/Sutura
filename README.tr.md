...
| Dolaylı predikatlar / Faz B arrangement-lite | ~%30 — deneysel | Rust tabanlı bir prototip (`--experimental-indirect-autorefine`), self-intersection geometrisini `--experimental-autorefine`'in kullandığı float64 EPICK snap-rounding yerine exact indirect predikatlarla böler. B1–B5 implemente edildi (predikat çekirdek, exact üçgen-üçgen sınıflandırıcı, implicit noktalı 2D CDT, `arrangement_lite` PyO3 bağlaması, `sutura/repair.py` kablolaması). Yoğun çok-segmentli host üçgenlerdeki "constrained edge blocks segment insertion" paniği, planar arrangement/PSLG yaklaşımına geçilerek düzeltildi. Bilinen sınırlar: varsayılan olarak kapalı; subdivision fazı yalnızca exact-rational olduğu için yoğun self-intersection taramaları (örn. thingi10k_1038441) pratik sürede bitmiyor; mevcut autorefine yolunun yerini alabilmesi için filtered f64/interval/exact değerlendirme katmanı gerekiyor. |

...
- `sutura/mesh_classifier.py` (stdlib+numpy only) heuristically classifies a mesh as `mechanical`/`organic`/`unknown` from dihedral-angle geometry. Its confidence is a **signed-margin score**: sigmoid memberships over both metrics (mechanical = max/OR of near90 & coplanar signals, organic = min/AND), decisions at fixed floors (`mechanical >= 0.7`, `organic >= 0.5`), and `unknown` carries a non-zero proximity value + `metrics['leaning']` instead of a flat 0. `repair_mesh_from_arrays` calls it and, on non-`unknown`, tunes two Stage 1 thresholds (`mincomponentsize`, `maxholesize`) via the module-level `_TYPE_PARAMS` table in `repair.py` — but only when confidence clears the **class-specific tuning gate** (`repair.MECH_TUNE_GATE=0.75`, `repair.ORG_TUNE_GATE=0.55`, helper `repair.tuning_applied_for`): below the gate the detected type is still reported but the conservative default thresholds are used, and the report carries `tuning_applied` (true/false; shown in `--human` and the GUI defect panel). The gate is class-specific because the organic confidence is structurally capped at ~0.62. Those per-type values are EXPERIMENTAL/uncalibrated starting points (see the code comment) — treat them as reversible. The result lands in the report as `detected_type`/`detected_confidence` and is shown in the GUI defect-panel header. Keep it free of pymeshlab/trimesh. Threshold resolution for BOTH the real repair and `--dry-run` goes through `repair.resolve_mode_params(mode, type, conf)` (returns `(params, tuning_applied)`), so they can never diverge.
